...
\item
  ...
  This transformation allows a process to quote a local term to be evaluated by
  an arbitrary role $u$. It happens in local mode ($\triangle$) and its
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    {
      \Gamma \vdash_m \primitive{leteval}_u\ x \bm{\leftarrow} s
      \primitive{ in } t(x) : B
    }
    \\
    ...
    {
      \Gamma \vdash_m \primitive{leteval}_u\ x \bm{\leftarrow} s
      \primitive{ in } t(x) \ \Longrightarrow\  \Gamma \vdash_m
      \primitive{letmod } y \bm{\leftarrow} s \primitive{ in } t(x)
      [ y^{\textsf{eval}_u} / x ]
    }
  \end{mathpar}
...
\begin{example}
  ...
  \begin{align*}
    ...
    \ & \primitive{letmod } y \bm{\leftarrow} x\\[-3pt]
      ...
      & \primitive{in } \primitive{mod } z^{\textsf{id}}
  \end{align*}
\end{example}
\noindent

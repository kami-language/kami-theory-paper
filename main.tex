...
represent unevaluated programs for metaprogramming\cite{bak2020self} and staged
...
in choreographic programming languages\cite{giallorenzo2024choral}, and it
stands to reason that existing research on modalities could provide a stepping
stone for the design of type systems for such languages.
...
typed lambda calculus with modalities, a variant of Modal Type
Theory\cite{gratzer2023syntax}. This requires introducing two new concepts:
common knowledge between multiple participants, and the representation of
global choreographies as values local to individual participants.
...
Cruz-Filipe et~al., and extends the simply typed lambda calculus (STLC) with
location annotations. For example, $t : \textsf{Bool} @ r$ means that $t$
...
Modal Type Theory (MTT), introduced by Gratzer in his PhD thesis, is a
framework for constructing modal type theories, with Martin-L\"{of} type theory
at its core. The framework is parametrized by a system of modalities specified
in the form of a \emph{mode theory}. MTT includes so-called ``full-spectrum''
dependent types, but we restrict ourselves to a simply typed fragment of MTT.
...
\begin{itemize}
\item
  The modality introduction rule \textsc{Mod-Intro} enforces that terms of a
  modal type $\langle A | \mu \rangle$ may only depend on variables that are
  themselves under the $\mu$ modality. Values of this type are eliminated using
  the modality elimination \textsc{Mod-Elim} rule, which allows us to assume
  $(A | \mu)$. The \emph{framing modality} $\nu$, omitted when equal to
  identity, allows the type $A$ to exist at a different mode than the motive
  $B$.
\item
...
\item
  All other terms are mode-local, and any modalities that occur in the terms
  are identities.
\end{itemize}
...
\item
  ...
  computations and data that occur at all roles. The local mode $\triangle$
  represents the perspective of a single location (which may be the conjunction
  of multiple roles) participating in the choreography.
\item
...
involving all roles.\footnote{For the sake of brevity, our mode theory
  ...
  which roles are involved in a given term of global type. It can be done by
...
under a modality'' as \emph{crisp induction principles}\cite[Section
6.3.1]{gratzer2023syntax}.

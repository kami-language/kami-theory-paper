...
dedicated term for communications that have not been performed yet. In our
semantics only $\textsf{eval}_v$ involves communications, so the only rule we
need to add is \textsc{Let-Eval} (figure \ref{fig:mtt_leteval}).

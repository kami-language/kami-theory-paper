...
\begin{figure}[h]
  ...
  \begin{mathpar}
    ...
    \inferrule*[Lab=Mod-Elim]
    {
      ...
      \\
      \Gamma.(x : A|\mu) \vdash_m t : B
    }
    {
      \Gamma \vdash_m \primitive{letmod } x \leftarrow s \primitive{ in } t : B
    }
    \\
    \inferrule*[Lab=Fun-Intro]
    {\Gamma.(x : A|\mu) \vdash_n t : B}
    {\Gamma \vdash_n \pmb{\lambda\,} x \primitive{\,.\,} t : (A | \mu) \to B}
    ...
    {\Gamma \vdash_m s \primitive{\,,\,} t : A \times B}
    ...
    {\Gamma \vdash_m \primitive{case } s \primitive{ of left } x
      \mapsto t \primitive{\,;\,right } y \mapsto u : C}
\end{mathpar}
...
\begin{itemize}
\item There are two modes: $\bigcirc$ and
  $\triangle$. The global mode $\bigcirc$ represents the
...
\item For each location $u \in \textrm{Loc}_\rho$ there is a modality $@u : \triangle \to
  \bigcirc$. Each of these modalities represents a different way of how a local
  computation can be embedded in the global system. Concretely, $@u$ expresses
  that the computation exists at location $u$.
\item An additional modality $\square : \bigcirc \to \triangle$, allowing global
...
\item For each location $v \in \textrm{Loc}_\rho$, a transformation
  $\textsf{eval}_v : (\square ; {}@ v) \Rightarrow \textsf{id}_\bigcirc$. This
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \inferrule*[Lab=Let-Eval]
    {
      ...
      \\
      \Gamma.(x : A|\textsf{id}_\bigcirc) \vdash s : B
    }
    {
...
\begin{definition}
  ...
  \item For each transformation $\alpha : \mu \Rightarrow \nu \in \mathcal{M}$,
    ...
    $\tau_\alpha : \iota_m \circ G(\mu) \Rightarrow \iota_m \circ G(\nu)$ in the % TODO: \circ probably needs to be changed to \fatsemi; check this
    target category.
  \end{enumerate}
\end{definition}
...
{}@ v) \Rightarrow \textsf{id}_\bigcirc$ is built up from the freely adjoined

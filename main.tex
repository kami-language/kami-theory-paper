...
\item
  ...
  available in quoted form at location $u$, can be scheduled to be executed by
  all involved roles.
\item
...
\begin{example}
  ...
  \begin{align*}
    \textsf{com}_{A,u,v}\ x =
    \ & \primitive{let}_{\textsf{id}}\:\primitive{mod}_{@u}\ y \bm{\leftarrow} x\\[-3pt]
      & \primitive{let}_{\textsf{id}}\:\primitive{eval}_u\ z \bm{\leftarrow} \primitive{mod}_{@v}\ (\primitive{mod}_{(\square \fatsemi @u)}\ 

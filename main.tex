...
{\tikz[baseline={([yshift=-.78ex]current bounding box.center)}]%
  {\node[circle,inner sep=0.4,outer sep=0,draw]%
    {$\displaystyle \phantom{o}$}}}%
{\tikz[baseline={([yshift=-.78ex]current bounding box.center)}]%
  {\node[circle,inner sep=0.4,outer sep=0,draw]%
    {$\textstyle \phantom{o}$}}}%
{\tikz[baseline={([yshift=-.78ex]current bounding box.center)}]%
  {\node[circle,inner sep=0.4,outer sep=0,draw]%
    {$\scriptstyle \phantom{o}$}}}%
{\tikz[baseline={([yshift=-.78ex]current bounding box.center)}]%
  {\node[circle,inner sep=0.4,outer sep=0,draw]%
    {$\scriptscriptstyle \phantom{o}$}}}}%
...
\item
  ...
  originates at the current mode, then $\mu = \textsf{id}_m : m \to m$.
\item
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    {\ }
    {\primitive{zero} : (A|\mu) \in_{\textsf{id}_m} \Gamma.(A | \mu)}
...
\begin{itemize}
\item
  There are new terms for introduction and elimination of modal types. The
  introduction rule \textsc{Mod-Intro} enforces that terms of a modal type
...
\item
  The function type introduction rule \textsc{Fun-Intro} preserves the
  additional modality by putting it into the context, alongside the new
  variables' type.
\item
  The function type elimination rule \textsc{Fun-Intro} deals with its modality
...
\begin{figure}[h]
  ...
  \begin{mathpar}
    ...
    {
      \Gamma \vdash_m s : \langle A | \mu \rangle
      \\
      \Gamma.(x : A|\mu) \vdash_m t(x) : B
    }
    {
      \Gamma \vdash_m \primitive{letmod } x \bm{\leftarrow} s \primitive{ in }
        t(x) : B
    }
    \\
    \inferrule*[Lab=Fun-Intro]
    {\Gamma.(x : A|\mu) \vdash_n t(x) : B}
    {\Gamma \vdash_n \bm{\lambda\,} x \bm{\,.\,} t(x) : (A | \mu) \to B}

    \inferrule*[Lab=Fun-Elim]
    {
      \Gamma \vdash_m t : (A | \mu) \to B
      \\
      \Gamma \vdash_n s : A
    }
    {\Gamma \vdash_n t\,s : B}
    ...
    {\Gamma \vdash_m s : A + B
      \\
      \Gamma.(x : A|\textsf{id}_m) \vdash_m t(x) : C
      \\
      \Gamma.(y : B|\textsf{id}_m) \vdash_m u(y) : C
    }
    {\Gamma \vdash_m \primitive{case } s \primitive{ of }
      \primitive{left } x \bm{\,.\,} t(x) \bm{\,;\,}
      \primitive{right } y \bm{\,.\,} u(y) : C}
  \end{mathpar}
...
\item
  ...
  of multiple roles. We do so by allowing arbitrary conjunctions $r_1 \land
  \ldots \land r_k$ in the modality. For example, the type $\langle \mathbb{N}
  | @(r\land s) \rangle$ expresses the fact that there is a natural number that
  is known by both roles $r$ and $s$.
\item
  We need roles to reference data that is about to be sent to other nodes. For
  this we use a new modality $\square$\footnote{Usually this symbol is
  pronounced ``box'', but we also refer to it by ``quote''.}, which allows
  roles to reference global choreographies locally. That is, if $A$ is a global
  choreography type, then $t : \langle A | \square \rangle$ is a local term,
  containing a quoted representation of such a choreography.
\end{enumerate}
...
\item
  ...
  $\textsf{quote}_u : \textsf{id}_\triangle \Rightarrow ({}@ u ; \square)$.
  This transformation allows a process to quote a local term to be evaluated
  by an arbitrary role $u$. It happens in local mode ($\triangle$) and its
  interpretation involves no communication.
\item
  For each location $v \in \textrm{Loc}_\rho$, a transformation
  $\textsf{eval}_v : (\square ; {}@ v) \Rightarrow \textsf{id}_{\circle}$. This
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    {
      \Gamma \vdash_{\circle} s : \langle A | \square \fatsemi @u \rangle
      \\
      \Gamma.(x : A|\textsf{id}_{\circle}) \vdash_m t(x) : B
    }
    {
      \Gamma \vdash_m \primitive{leteval}_u\ x \leftarrow s \primitive{ in }
    t(x) : B
    }
    \\
    ...
    {
      \Gamma \vdash_m \primitive{leteval}_u\ x \leftarrow s \primitive{ in }
      t(x) \rightsquigarrow \Gamma \vdash_m \primitive{letmod } y \leftarrow s
      \primitive{ in } t(x) [ y^{\textsf{eval}_u} / x ]
    }
  \end{mathpar}
...
\begin{definition}
  ...
  \begin{enumerate}
  \item
    Initializing simply typed MTT with $\mathcal{M}^{\rho}_{\textrm{Chor}}$.
  \item
    Extending it with the \textsc{Let-Eval} rule.
  \item
    Restricting the reduction relation analogously how Chor$\lambda$ restricts
    the reduction relation of STLC.
  \end{enumerate}
\end{definition}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Behavioural semantics and relation with
  ...
  \begin{align*}\setlength{\jot}{0.5ex}
    ...
    \ & \primitive{letmod}\ a_1 \leftarrow a\\
      & \primitive{leteval}_u\ a_2 \leftarrow
          \primitive{mod}\ a_1^{\textsf{quote}_v * \textsf{id}_{@u}}\\
...
\begin{definition}
  ...
  \item
    ...
    \iota_m \circ G(\nu)$ in the target category. % TODO: \circ probably needs to be changed to \fatsemi; check this
  \end{enumerate}
\end{definition}
\noindent
...
(\square ; {}@ v) \Rightarrow \textsf{id}_{\circle}$ is built up from the
